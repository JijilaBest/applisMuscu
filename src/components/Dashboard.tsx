...
import { mockHistory, ronanHistory } from '../store/mockData';
...
const Dashboard: React.FC<DashboardProps> = ({ currentUser }) => {
  ...
  // Use correct history based on current user
  const history = currentUser.id === 'user-1' ? mockHistory : ronanHistory;
      ...
      <div className="glass-card flex-col gap-4 mt-4">
        ...
        <div className="flex" style={{ background: 'var(--bg-main)', padding: '4px', borderRadius: '8px', gap: '4px' }}>
          <button 
            ...
            style={{ flex: 1, padding: '0.5rem', borderRadius: '4px', background: metric === 'volume' ? 'var(--primary)' : 'transparent', color: metric === 'volume' ? '#fff' : 'var(--text-muted)', fontWeight: 700, transition: 'all 0.2s', fontSize: '0.85rem' }}
          >
            Volume (kg)
          </button>
          <button 
            onClick={() => setMetric('reps')}
            style={{ flex: 1, padding: '0.5rem', borderRadius: '4px', background: metric === 'reps' ? 'var(--primary)' : 'transparent', color: metric === 'reps' ? '#fff' : 'var(--text-muted)', fontWeight: 700, transition: 'all 0.2s', fontSize: '0.85rem' }}
          >
            Séries/Reps
          </button>
          <button 
            onClick={() => setMetric('max')}
            style={{ flex: 1, padding: '0.5rem', borderRadius: '4px', background: metric === 'max' ? 'var(--primary)' : 'transparent', color: metric === 'max' ? '#fff' : 'var(--text-muted)', fontWeight: 700, transition: 'all 0.2s', fontSize: '0.85rem' }}
          >
            Max (kg)
          </button>
        </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
              ...
              <Tooltip 
                ...
                itemStyle={{ color: 'var(--primary)', fontFamily: 'var(--font-heading)', fontSize: '1.2rem' }}
              />
              <Line 
                type="monotone" 
                dataKey="value" 
                stroke="var(--primary)" 
                strokeWidth={3}
                dot={{ r: 6, fill: 'var(--bg-secondary)', stroke: 'var(--primary)', strokeWidth: 2 }}
          ...
          {history.slice(0, 3).map(session => (
            <div key={session.id} className="glass-card flex items-center justify-between" style={{ padding: '1rem' }}>
              <div className="flex items-center gap-3">
                <div className="btn-icon" style={{ width: '40px', height: '40px', background: 'rgba(160, 32, 240, 0.1)' }}>
